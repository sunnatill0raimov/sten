...
class Crypto {
	...
	static accessSten(encryptedData, passwordHash, password) {
		try {
			...
			// Verify password
			const isPasswordValid = Hashing.verifyPasswordWithSecret(
				password,
				passwordHashObj
			)
			if (!isPasswordValid) {
		...
		try {
			...
			if (passwordStrength.strength === 'weak') {
				console.warn(
					...
					passwordStrength.recommendations
				)
			}
	...
	static validateStenData(stenData) {
		...
		const hasAllFields = requiredFields.every(
			field => stenData[field] && typeof stenData[field] === 'string'
		)
		...
		try {
			...
			return (
				Encryption.validateEncryptedData(encryptedDataObj) &&
...
export { CRYPTO_CONFIG, Encryption, Hashing, KeyManager }

// Export specific methods for convenience
export const encryptMessage = Encryption.encryptMessage.bind(Encryption)
export const encryptMessageWithPassword = Encryption.encryptMessageWithPassword.bind(Encryption)
export const hashPasswordOnly = Crypto.hashPasswordOnly.bind(Crypto)
export const verifyPasswordOnly = Crypto.verifyPasswordOnly.bind(Crypto)
export const decryptMessageWithPassword = Crypto.decryptMessageWithPassword.bind(Crypto)

...
import type { StenContent, StenMetadata } from '../api/stenApi'
import { getStenMetadata, unlockSten, viewSten } from '../api/stenApi'

type StenState = 'loading' | 'not_found' | 'destroyed' | 'expired' | 'active'
...
const ViewSten: React.FC = () => {
		...
		const loadSten = async () => {
			try {
				...
				if (!metadataResponse.exists) {
					...
					metadataResponse.reason === 'destroyed' ||
					metadataResponse.destroyed
				) {
			...
			<div className='flex items-center justify-center min-h-screen px-6 py-8'>
				<div className='w-full max-w-[640px]'>
					<div className='bg-[#111111] border border-[rgba(255,255,255,0.08)] rounded-2xl p-8 text-center'>
						...
						<p className='text-white/60 mb-6'>This STEN has expired.</p>
						{metadata?.expiresAt && (
	...
	if (stenState === 'active') {
		...
		const isPasswordProtected =
			metadata?.requiresPassword || metadata?.isPasswordProtected
								...
								<div className='mb-6 p-4 bg-yellow-500/20 border border-yellow-500/30 rounded-xl'>
									<div className='flex items-center gap-3'>
										...
										<p className='text-yellow-400 text-sm font-medium'>
											This STEN will be permanently destroyed after closing this
											page.
										</p>
									</div>
								</div>

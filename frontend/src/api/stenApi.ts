...
export interface StenData {
  ...
  maxViews: number | null; // null means unlimited views
}
 ...
 */
export const viewSten = async (id: string, password?: string): Promise<StenContent> => {
  try {
 ...
 */
export const solveSten = async (id: string, password?: string): Promise<StenContent> => {
  return viewSten(id, password);
};
...
export const getStenStatus = async (id: string): Promise<{ status: string }> => {
  try {
    ...
    if (metadata.expired) {
      status = 'expired';
    } else if (typeof metadata.viewsRemaining === 'number' && metadata.viewsRemaining <= 0) {
